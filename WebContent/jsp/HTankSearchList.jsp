...
<HTML><HEAD><TITLE>Hydraulic Tank Search</TITLE>
...
function setInfo(){
	...
	if (radioChecked(formObj) == "false"){
		alert("Please select a Hydraulic Tank.");
	}else{
		var id ="";
		var idName="" ;
		if (document.forms[0].stackRsltRbtn && document.forms[0].stackRsltRbtn.length){
			...
			if(document.forms[0].stackRsltRbtn){		
				...
				alert("No Hydraulic Tank Available.");
			}
			
		}
		opener.document.forms[0].<bean:write name="HtankListInfo" property="formId" />.value=id;
		opener.document.forms[0].<bean:write name="HtankListInfo"  property="formIdName"  />.value=idName;
		self.close();		
	}
}
</script>
...
<BODY>
	<html:form action="/HTankListInfo">			
<BR><BR>
		<eespc:table dataSource="HTANK_SEARCH_LIST"
		  formName="form1"
		  noDataFoundMessage="<CENTER><SPAN CLASS=error>No Hydraulic Tank available in this facility.</SPAN></CENTER>" >
		  
		  <eespc:tableHeaderCellDef id="h_Capacity" label="CAPACITY" bodyCellId="b_capacity"/>		
		  <eespc:tableHeaderCellDef id="h_HId" label="Hydralic Storage Tank ID (Facility Designated)" bodyCellId="b_HId"/>			  		  
