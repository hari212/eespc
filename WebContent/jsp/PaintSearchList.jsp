...
function setInfo(){
	...
	if (radioChecked(formObj) == "false"){
		alert("Please select a Paint Spray Booth.");
	}else{
		var id ="";
		var idName="" ;
		if (document.forms[0].stackRsltRbtn && document.forms[0].stackRsltRbtn.length){
			...
			if(document.forms[0].stackRsltRbtn){		
				...
				alert("No Paint Spray Booth Available.");
			}
...
<BODY>
	<html:form action="/PaintListInfo">			
<BR><BR>
		<eespc:table dataSource="PAINT_SEARCH_LIST"
		  formName="form1"
		  noDataFoundMessage="<CENTER><SPAN CLASS=error>No Paint Spray Booth available in this facility.</SPAN></CENTER>" >
