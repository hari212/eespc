...
<HTML><HEAD><TITLE>Place of Assembly Search</TITLE>
...
function setInfo(){
	...
	if (radioChecked(formObj) == "false"){
		alert("Please select a PlaceofAssembly Source.");
	}else{
		var id ="";
		var idName="" ;
		if (document.forms[0].stackRsltRbtn && document.forms[0].stackRsltRbtn.length){
			...
			if(document.forms[0].stackRsltRbtn){		
				...
				alert("No PlaceofAssembly Source Available.");
			}
...
<BODY>
	<html:form action="/PlaceofAssemblyListInfo">			
<BR><BR>
		<eespc:table dataSource="PLACEOFASSEMBLY_SEARCH_LIST"
		  formName="form1"
		  noDataFoundMessage="<CENTER><SPAN CLASS=error>No PlaceofAssembly Unit Source available in this facility.</SPAN></CENTER>" >
