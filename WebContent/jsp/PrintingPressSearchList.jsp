...
function setInfo(){
	...
	if (radioChecked(formObj) == "false"){
		alert("Please select a Printing Press.");
	}else{
		var id ="";
		var idName="" ;
		if (document.forms[0].stackRsltRbtn && document.forms[0].stackRsltRbtn.length){
			...
			if(document.forms[0].stackRsltRbtn){		
				if(document.forms[0].stackRsltRbtn.checked){
					...
					alert("Please select a Press.");
				}
			}else{
				alert("No Press Available.");
			}
...
<BODY>
	<html:form action="/PressListInfo">			
<BR><BR>
		<eespc:table dataSource="PRESS_SEARCH_LIST"
		  formName="form1"
		  noDataFoundMessage="<CENTER><SPAN CLASS=error>No Printing Press available in this facility.</SPAN></CENTER>" >
