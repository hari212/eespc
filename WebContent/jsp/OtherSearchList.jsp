...
<HTML><HEAD><TITLE>Printing Press Search</TITLE>
...
function setInfo(){
	...
	if (radioChecked(formObj) == "false"){
		alert("Please select a Miscellaneous Source.");
	}else{
		var id ="";
		var idName="" ;
		if (document.forms[0].stackRsltRbtn && document.forms[0].stackRsltRbtn.length){
			...
			if(document.forms[0].stackRsltRbtn){		
				...
				alert("No Miscellaneous Source Available.");
			}
...
<BODY>
	<html:form action="/OtherListInfo">			
<BR><BR>
		<eespc:table dataSource="OTHER_SEARCH_LIST"
		  formName="form1"
		  noDataFoundMessage="<CENTER><SPAN CLASS=error>No Miscellaneous Source available in this facility.</SPAN></CENTER>" >
		  ...
		  <eespc:tableBodyCellDef id="b_HId" method="getFacilitydesinatedId"/>			  
		  <eespc:tableBodyCellDef id="b_buildingName" method="getBuildingName"/>	 	  
		  <eespc:tableBodyCellDef id="b_HId2" method="getFacilitydesinatedId"/>			
		  <eespc:tr>
