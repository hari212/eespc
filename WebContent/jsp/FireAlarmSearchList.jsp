...
function setInfo(){
	...
	if (radioChecked(formObj) == "false"){
		alert("Please select a FireAlarm Source.");
	}else{
		var id ="";
		var idName="" ;
		if (document.forms[0].stackRsltRbtn && document.forms[0].stackRsltRbtn.length){
			...
			if(document.forms[0].stackRsltRbtn){		
				...
				alert("No FireAlarm Source Available.");
			}
...
<BODY>
	<html:form action="/FireAlarmListInfo">			
<BR><BR>
		<eespc:table dataSource="FIREALARM_SEARCH_LIST"
		  formName="form1"
		  noDataFoundMessage="<CENTER><SPAN CLASS=error>No FireAlarm Source available in this facility.</SPAN></CENTER>" >
