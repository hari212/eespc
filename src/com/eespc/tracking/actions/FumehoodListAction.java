...
public class FumehoodListAction extends Action {

	private static Log log = LogFactory
			.getLog(com.eespc.tracking.actions.FumehoodListAction.class);

	public FumehoodListAction() {
	}
	...
	private void getHydraulicTankListInfo(HttpServletRequest httpservletrequest)
		...
		java.util.List list = FumehoodEntity.getFacilityStackList(facilityvo
				.getId());
		System.out.println((new StringBuilder()).append("List=").append(list)
				.toString());
		if (list != null) {
			httpsession.setAttribute("FUMEHOOD_SEARCH_LIST", list);
		} else {
			httpsession.setAttribute("FUMEHOOD_SEARCH_LIST", new ArrayList());
		}
	}
