...
public class IncineratorListAction extends Action {

	private static Log log = LogFactory
			.getLog(com.eespc.tracking.actions.IncineratorListAction.class);

	public IncineratorListAction() {
	}
	...
	private void getHydraulicTankListInfo(HttpServletRequest httpservletrequest)
		...
		java.util.List list = IncineratorEntity.getFacilityStackList(facilityvo
				.getId());
		System.out.println((new StringBuilder()).append("List=").append(list)
				.toString());
		if (list != null) {
			httpsession.setAttribute("INCINERATOR_SEARCH_LIST", list);
		} else {
			httpsession
					.setAttribute("INCINERATOR_SEARCH_LIST", new ArrayList());
		}
	}
