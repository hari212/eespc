...
public class ChillerRefrigationListAction extends Action {

	public ChillerRefrigationListAction() {
	}
	...
	private void getHydraulicTankListInfo(HttpServletRequest httpservletrequest)
		...
		java.util.List list = ChillerRefrigationEntity
				.getFacilityStackList(facilityvo.getId());

		System.out.println((new StringBuilder()).append("List=").append(list)
				.toString());
		if (list != null)
			httpsession.setAttribute("CHILLERREFRIGATION_SEARCH_LIST", list);
		else
			httpsession.setAttribute("CHILLERREFRIGATION_SEARCH_LIST",
					new ArrayList());
	}

	private static Log log = LogFactory
			.getLog(com.eespc.tracking.actions.ChillerRefrigationListAction.class);
