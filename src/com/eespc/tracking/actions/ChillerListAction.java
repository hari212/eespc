...
public class ChillerListAction extends Action {

	private static Log log = LogFactory
			.getLog(com.eespc.tracking.actions.ChillerListAction.class);

	public ChillerListAction() {
	}
	...
	private void getHydraulicTankListInfo(HttpServletRequest httpservletrequest)
		...
		java.util.List list = ChillerEntity.getFacilityStackList(facilityvo
				.getId());
		System.out.println((new StringBuilder()).append("List=").append(list)
				.toString());
		if (list != null) {
			httpsession.setAttribute("CHILLER_SEARCH_LIST", list);
		} else {
			httpsession.setAttribute("CHILLER_SEARCH_LIST", new ArrayList());
		}
	}
