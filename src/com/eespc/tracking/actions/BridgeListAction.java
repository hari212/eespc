...
public class BridgeListAction extends Action {
	...
	private void getHydraulicTankListInfo(HttpServletRequest httpservletrequest)
		...
		java.util.List list = BridgeTunnelsEntity
				.getFacilityStackList(facilityvo.getId());
		System.out.println((new StringBuilder()).append("List=").append(list)
				.toString());
		if (list != null)
			httpsession.setAttribute("BRIDGE_SEARCH_LIST", list);
		else
			httpsession.setAttribute("BRIDGE_SEARCH_LIST", new ArrayList());
	}

	private static Log log = LogFactory
			.getLog(com.eespc.tracking.actions.BridgeListAction.class);
