...
public class RPZListAction extends Action {

	private static Log log = LogFactory
			.getLog(com.eespc.tracking.actions.RPZListAction.class);

	public RPZListAction() {
	}
	...
	private void getHydraulicTankListInfo(HttpServletRequest httpservletrequest)
		...
		java.util.List list = RpzEntity
				.getFacilityStackList(facilityvo.getId());
		System.out.println((new StringBuilder()).append("List=").append(list)
				.toString());
		if (list != null) {
			httpsession.setAttribute("RPZ_SEARCH_LIST", list);
		} else {
			httpsession.setAttribute("RPZ_SEARCH_LIST", new ArrayList());
		}
	}
