...
public class TankExternalEnum implements Serializable, Comparable {
	...
	private TankExternalEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private TankExternalEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static TankExternalEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new TankExternalEnum(const2name);
	}

	public static TankExternalEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			TankExternalEnum tankexternalenum = new TankExternalEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s1 = "";
			if (tankexternalenum != null) {
				String s2 = (new StringBuffer(String.valueOf(tankexternalenum
						.getCode()))).toString();
				String s3 = tankexternalenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof TankExternalEnum) {
			TankExternalEnum tankexternalenum = (TankExternalEnum) obj;
			return getName().compareToIgnoreCase(tankexternalenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names
				.getInstance(ConstantGroup.TANK_EXTERNAL_PROTECT);
	}
}
