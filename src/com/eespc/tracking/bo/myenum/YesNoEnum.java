...
public class YesNoEnum implements Serializable, Comparable {
	...
	public static YesNoEnum get(int i) {
		...
		return const2name == null ? null : new YesNoEnum(const2name);
	}

	public static YesNoEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			YesNoEnum yesnoenum = new YesNoEnum((Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (yesnoenum != null) {
				String s1 = (new StringBuffer(String.valueOf(yesnoenum
						.getCode()))).toString();
				String s3 = yesnoenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof YesNoEnum) {
			YesNoEnum yesnoenum = (YesNoEnum) obj;
			return getName().compareToIgnoreCase(yesnoenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.YESNO);
	}
}
