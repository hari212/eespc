...
public class OverfillPreventionEnum implements Serializable, Comparable {
	...
	private OverfillPreventionEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private OverfillPreventionEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static OverfillPreventionEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new OverfillPreventionEnum(
				const2name);
	}

	public static OverfillPreventionEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			OverfillPreventionEnum overfillpreventionenum = new OverfillPreventionEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (overfillpreventionenum != null) {
				String s1 = (new StringBuffer(
						String.valueOf(overfillpreventionenum.getCode())))
						.toString();
				String s3 = overfillpreventionenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof OverfillPreventionEnum) {
			OverfillPreventionEnum overfillpreventionenum = (OverfillPreventionEnum) obj;
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.OVERFILL_PREVENTION);
	}
}
