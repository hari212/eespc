...
public class ChillerOperatingEnum implements Serializable, Comparable {
	...
	private ChillerOperatingEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private ChillerOperatingEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static ChillerOperatingEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name != null ? new ChillerOperatingEnum(const2name) : null;
	}

	public static ChillerOperatingEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			ChillerOperatingEnum chilleroperatingenum = new ChillerOperatingEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s1 = "";
			if (chilleroperatingenum != null) {
				String s2 = (new StringBuffer(
						String.valueOf(chilleroperatingenum.getCode())))
						.toString();
				String s3 = chilleroperatingenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof ChillerOperatingEnum) {
			ChillerOperatingEnum chilleroperatingenum = (ChillerOperatingEnum) obj;
			return getName()
					.compareToIgnoreCase(chilleroperatingenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.CHILLER_OPERATED_BY);
	}
}
