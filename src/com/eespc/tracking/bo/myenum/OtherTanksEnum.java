...
public class OtherTanksEnum implements Serializable, Comparable {
	...
	private OtherTanksEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private OtherTanksEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static OtherTanksEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name != null ? new OtherTanksEnum(const2name) : null;
	}

	public static OtherTanksEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			OtherTanksEnum othertanksenum = new OtherTanksEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s1 = "";
			if (othertanksenum != null) {
				String s2 = (new StringBuffer(String.valueOf(othertanksenum
						.getCode()))).toString();
				String s3 = othertanksenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof OtherTanksEnum) {
			OtherTanksEnum othertanksenum = (OtherTanksEnum) obj;
			return getName().compareToIgnoreCase(othertanksenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.OTHER_TANKS);
	}
}
