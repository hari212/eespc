...
public class HydraulicStorageUsageEnum implements Serializable, Comparable {
	...
	private HydraulicStorageUsageEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private HydraulicStorageUsageEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static HydraulicStorageUsageEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new HydraulicStorageUsageEnum(
				const2name);
	}

	public static HydraulicStorageUsageEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			HydraulicStorageUsageEnum hydraulicstorageusageenum = new HydraulicStorageUsageEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (hydraulicstorageusageenum != null) {
				String s1 = (new StringBuffer(
						String.valueOf(hydraulicstorageusageenum.getCode())))
						.toString();
				String s3 = hydraulicstorageusageenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof HydraulicStorageUsageEnum) {
			HydraulicStorageUsageEnum hydraulicstorageusageenum = (HydraulicStorageUsageEnum) obj;
	...
	static {
		constants = Const2Names
				.getInstance(ConstantGroup.HYDRAULIC_STORAGE_USAGE);
	}
}
