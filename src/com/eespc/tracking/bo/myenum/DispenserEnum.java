...
public class DispenserEnum implements Serializable, Comparable {
	...
	private DispenserEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private DispenserEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static DispenserEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new DispenserEnum(const2name);
	}

	public static DispenserEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			DispenserEnum dispenserenum = new DispenserEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s1 = "";
			if (dispenserenum != null) {
				String s2 = (new StringBuffer(String.valueOf(dispenserenum
						.getCode()))).toString();
				String s3 = dispenserenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof DispenserEnum) {
			DispenserEnum dispenserenum = (DispenserEnum) obj;
			return getName().compareToIgnoreCase(dispenserenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.TANK_DISPENSER);
	}
}
