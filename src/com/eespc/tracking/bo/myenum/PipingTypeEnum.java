...
public class PipingTypeEnum implements Serializable, Comparable {
	...
	private PipingTypeEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private PipingTypeEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static PipingTypeEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new PipingTypeEnum(const2name);
	}

	public static PipingTypeEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			PipingTypeEnum pipingtypeenum = new PipingTypeEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (pipingtypeenum != null) {
				String s1 = (new StringBuffer(String.valueOf(pipingtypeenum
						.getCode()))).toString();
				String s3 = pipingtypeenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof PipingTypeEnum) {
			PipingTypeEnum pipingtypeenum = (PipingTypeEnum) obj;
			return getName().compareToIgnoreCase(pipingtypeenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.PIPING_TYPE);
	}
}
