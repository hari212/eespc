...
public class SecondaryContainmentEnum implements Serializable, Comparable {
	...
	private SecondaryContainmentEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private SecondaryContainmentEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static SecondaryContainmentEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new SecondaryContainmentEnum(
				const2name);
	}

	public static SecondaryContainmentEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			SecondaryContainmentEnum secondarycontainmentenum = new SecondaryContainmentEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (secondarycontainmentenum != null) {
				String s1 = (new StringBuffer(
						String.valueOf(secondarycontainmentenum.getCode())))
						.toString();
				String s3 = secondarycontainmentenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof SecondaryContainmentEnum) {
			SecondaryContainmentEnum secondarycontainmentenum = (SecondaryContainmentEnum) obj;
	...
	static {
		constants = Const2Names
				.getInstance(ConstantGroup.SECONDARY_CONTAINMENT);
	}
}
