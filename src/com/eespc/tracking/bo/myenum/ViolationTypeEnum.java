...
public class ViolationTypeEnum implements Serializable, Comparable {
	...
	private ViolationTypeEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private ViolationTypeEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static ViolationTypeEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new ViolationTypeEnum(const2name);
	}

	public static ViolationTypeEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			ViolationTypeEnum violationtypeenum = new ViolationTypeEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (violationtypeenum != null) {
				String s1 = (new StringBuffer(String.valueOf(violationtypeenum
						.getCode()))).toString();
				String s3 = violationtypeenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof ViolationTypeEnum) {
			ViolationTypeEnum violationtypeenum = (ViolationTypeEnum) obj;
			return getName().compareToIgnoreCase(violationtypeenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.VIOLATION_TYPE);
	}
}
