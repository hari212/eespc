...
public class ExternalProtectionEnum implements Serializable, Comparable {
	...
	private ExternalProtectionEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private ExternalProtectionEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static ExternalProtectionEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name == null ? null : new ExternalProtectionEnum(
				const2name);
	}

	public static ExternalProtectionEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			ExternalProtectionEnum externalprotectionenum = new ExternalProtectionEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s2 = "";
			if (externalprotectionenum != null) {
				String s1 = (new StringBuffer(
						String.valueOf(externalprotectionenum.getCode())))
						.toString();
				String s3 = externalprotectionenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof ExternalProtectionEnum) {
			ExternalProtectionEnum externalprotectionenum = (ExternalProtectionEnum) obj;
	...
	static {
		constants = Const2Names.getInstance(ConstantGroup.EXTERNAL_PROTECTION);
	}
}
