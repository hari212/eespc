...
public class TankCorrosionEnum implements Serializable, Comparable {
	...
	private TankCorrosionEnum(Const2Name const2name) {
		code = const2name.getValue();
		name = const2name.getName();
		descriptor = const2name.getDescription();
	}

	private TankCorrosionEnum(int i, String s, String s1) {
		code = i;
		name = s;
		descriptor = s1;
	}
	...
	public static TankCorrosionEnum get(int i) {
		Const2Name const2name = constants.get(i);
		return const2name != null ? new TankCorrosionEnum(const2name) : null;
	}

	public static TankCorrosionEnum parse(String s) {
	...
	public static DropDown getDropDownObj() {
		...
		for (int j = 0; j < i; j++) {
			TankCorrosionEnum tankcorrosionenum = new TankCorrosionEnum(
					(Const2Name) list.get(j));
			String s = "";
			String s1 = "";
			if (tankcorrosionenum != null) {
				String s2 = (new StringBuffer(String.valueOf(tankcorrosionenum
						.getCode()))).toString();
				String s3 = tankcorrosionenum.getName();
	...
	public int compareTo(Object obj) {
		if (obj instanceof TankCorrosionEnum) {
			TankCorrosionEnum tankcorrosionenum = (TankCorrosionEnum) obj;
			return getName().compareToIgnoreCase(tankcorrosionenum.getName());
		} else {
			return -1;
		}
	}
	...
	static {
		constants = Const2Names
				.getInstance(ConstantGroup.TANK_CORROSIONPROTECTION);
	}
}
